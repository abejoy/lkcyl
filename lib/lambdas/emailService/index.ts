import { SESClient, SendEmailCommand, SendEmailCommandInput, SendRawEmailCommand, SendRawEmailCommandInput } from '@aws-sdk/client-ses'
import { GetObjectCommand, GetObjectCommandOutput, S3Client  } from '@aws-sdk/client-s3'
import { Color, Gender, Maybe, Player, Team } from '../helpers/types/graphql-types';
...
const sendEmail = async(ToAddresses: string[], subject: string = '', body:string = '') => {
...
const fetchS3Response = async(Key: string): Promise<GetObjectCommandOutput> =>{
...
const sendEmailWithAttachment = async (recipients:string[], subject:string, bodyHtml: string, attachmentFillePathsInBucket: string[]) => {
  try {
...
`;
    ...
    for(const attachmentFilePath of attachmentFillePathsInBucket) {
      ...
      const contentType = mime.lookup(attachmentFilePath) || "application/octet-stream";
...
`;
    ...
    throw new Error(`Failed to send email with attachement ${JSON.stringify(err)}`);
  }
}

const replaceStringArgs = (template: string = '', replacements: ReplacementArgs): string => {
  if(!template){
    return '';
  }
  let result = template;
...
const getHtmlBody = async (s3TextPath: string, replacementParams:ReplacementArgs): Promise<string|undefined> => {
...
const getAllPlayersAsFormatedString = (players: Maybe<Maybe<Player>[]> = []): string => {
  const playersString: string[] = players?.map(player => player?.name || '') || [];
...
export const handler = async (event: any) => {
  try {
    ...
    for(const emailToSend of emailsToSend) {
      ...
      const allPlayers: string = getAllPlayersAsFormatedString(emailToSend.emailArgs?.players);
      ...
      const teamDetails = getTeamDetailsText(emailToSend.emailArgs || 'undefined Team');

      switch(emailToSend.emailTemplate) {
        case EmailTemplate.Custom:
          emailSentData = await sendEmail(ToAddresses, emailToSend.emailSubject, emailToSend.emailBody);
          break;
        case EmailTemplate.Captian:
          const capReplacementParams: ReplacementArgs = { name: emailToSend.emailArgs?.captianName || '', teamName, unit, gender, allPlayers, role: emailToSend.emailTemplate, color, teamDetails};
          const htmlBody: string = await getHtmlBody('emailbody/captianEmail.txt', capReplacementParams) || '';
          emailSentData = await sendEmailWithAttachment(emailToSend.emailAddressToSend, sub, htmlBody, ['attachments/poster.jpeg']);
          break;
        case EmailTemplate.Manager:
          const managerReplacementParams: ReplacementArgs = { name: emailToSend.emailArgs?.managerName || '', teamName, unit, gender, allPlayers, role: emailToSend.emailTemplate, color, teamDetails};
          const managerHtmlBody: string = await getHtmlBody('emailbody/captianEmail.txt', managerReplacementParams) || '';
          emailSentData = await sendEmailWithAttachment(emailToSend.emailAddressToSend, sub, managerHtmlBody, ['attachments/poster.jpeg']);
          break;
        case EmailTemplate.Admin:
          const adminReplacementParams: ReplacementArgs = { name: emailToSend.emailArgs?.managerName || '', teamName, unit, gender, allPlayers, role: emailToSend.emailTemplate, color, teamDetails};
          const adminHtmlBody:string = await getHtmlBody('emailbody/admin.txt', adminReplacementParams) || '';
          emailSentData = await sendEmailWithAttachment(emailToSend.emailAddressToSend, `Team ${teamName} Registered`,adminHtmlBody ,[])
          break;
        default:
          break;
      }
